...
class Pipeline:
    ...
    def get_parsed_source(self, input_file: str,
        ...
        if output_file is None:
            ...
            if self.language == 'c':
                ...
                source['code'] = source['code'].progress_apply(parser.parse)#把源代码解析成<pycparser.c_ast.FileAST object at 0x0000020E1277B380>的形式
    ...
    def split_data(self):
        ...
        data = self.pairs#读取oj,里面只包含id1 id2 label
        data_num = len(data)
        ...
        #train.to_pickle(self.train_file_path)#分割出来的数据也只包含id1 id2 label
    ...
    def dictionary_and_embedding(self, input_file, size):
        ...
        pairs = pd.read_pickle(input_file)#train_.pkl，只包含id1 id2 label
        train_ids = pairs['id1'].append(pairs['id2']).unique()

        trees = self.sources.set_index('id', drop=False).loc[train_ids]#ast设置索引
    ...
    def run(self):
        ...
        if os.path.exists(os.path.join(self.root, self.language, 'ast.pkl')):#如果ast存在
            print('a')
            self.get_parsed_source(input_file='ast.pkl')
        else:
            self.get_parsed_source(input_file=input_file,#生成ast.pkl
                                   output_file='ast.pkl')
        print('read id pairs...')
        if self.language == 'c':
            self.read_pairs('oj_clone_ids.pkl')#令pairs = oj_clone_ids.pkl
        else:
